  ...
  if (findUnassigned(grid, &r, &c)) {
    for (int n = 1; n <= 9; n++) {
      if (isValid(grid, n, r, c)) {
        ...
        // i wish her love for me had rrecursion
        if (solveGrid(grid))
          return 1;
        grid[r][c] = UNASSIGNED;
      }
...
int isValid(int grid[N][N], int number, int r, int c) {
  if (grid[r][c] == UNASSIGNED && isValidInRow(grid, number, r) &&
      isValidInColumn(grid, number, c) &&
      isValidInBox(grid, number, r - r % 3, c - c % 3))
    return 1;
  return 0;
}
